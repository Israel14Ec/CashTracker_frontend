...
export default function DeleteBudgetModal() {
  ...
  const searchParams = useSearchParams(); //Obtiene los QueryStrings de la URl

  const deleteBudgetId = searchParams.get("deleteBudgetId");
  const show = !!deleteBudgetId;

  const hideModal = new URLSearchParams(searchParams.toString()); //URL search params es de JS
      ...
      <>
        <Transition appear show={show} as={Fragment}>
          <Dialog as="div" className="relative z-10" onClose={() => {
            router.replace(pathname + hideModal.toString())
          }}>
            <TransitionChild
              as={Fragment}
